...
class cumulDiamondODEModel(pints.ForwardModel):
    
    def simulate(self, parameters, times):
        # Unpack parameters
        bbarcp, cpp, b1, tpp, tcc, thetaa, thetap, chi = parameters

        Np = 2666
        Nc = 1045    
        # Initial conditions
        y0 = np.zeros(24)
        y0[0] = Np-1  # Initial no. susceptible passengers
        y0[1] = Nc    # Initial no. susceptible crew
        ...
        def ode_system(y, t, bbarcp, cpp, b1, tpp, tcc, thetaa, thetap, chi):
            return self.the_ode_model(t, y, bbarcp, cpp, b1, tpp, tcc, thetaa, thetap, chi)
        
        # Solve the ODE using odeint
        sol = odeint(ode_system, y0, times, args=(bbarcp, cpp, b1, tpp, tcc, thetaa, thetap, chi))
    ...
    @staticmethod
    def the_ode_model(t, y, bbarcp, cpp, b1, tpp, tcc, thetaa, thetap, chi):
        b2 = 80
        nu = 2 / 4.3
        ga = 1 / 5
        gp = 1 / 2.1
        ...
        ft = Ntests / (np.sum(y[[0, 1, 2, 3, 6, 7, 8, 9, 16, 17, 18, 19]]))
        ...
        dy[2] = y[0] * (bpp * thetaa * y[8] + bpp * thetap * y[6] + bpp * y[10] + bpp * y[14]) / 2666 + \
                y[0] * (bpc * thetaa * y[9] + bpc * thetap * y[7] + bpc * y[11] + bpc * y[15]) / 1045 - nu * y[2]
        dy[3] = y[1] * (bcp * thetaa * y[8] + bcp * thetap * y[6] + bcp * y[10] + bcp * y[14]) / 2666 + \
                y[1] * (bcc * thetaa * y[9] + bcc * thetap * y[7] + bcc * y[11] + bcc * y[15]) / 1045 - nu * y[3]
        dy[4] = nu * y[2] - nu * y[4]
        dy[5] = nu * y[3] - nu * y[5]
