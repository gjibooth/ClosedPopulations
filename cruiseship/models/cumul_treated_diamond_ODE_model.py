...
class cumulTreatedDiamondODEModel(pints.ForwardModel):
    
    def simulate(self, parameters, times, y0=None):
        # Unpack parameters
        ...
        # Use provided initial conditions if supplied, otherwise use default
        if y0 is None:
            # Initialize the array
            y0 = np.zeros(26)

            # Calculate initial susceptible and protected individuals
            initial_susceptible_passengers = (1 - pintp) * Np
            initial_protected_passengers = pintp * Np

            # Adjust if necessary to account for the symptomatic individual
            if initial_susceptible_passengers < 1:
                # Reduce the number of protected passengers instead
                y0[0] = initial_susceptible_passengers  # All susceptible passengers become symptomatic or protected
                y0[24] = initial_protected_passengers - 1  # Adjusting protected passengers
            else:
                y0[0] = initial_susceptible_passengers - 1  # Reserve one for symptomatic passenger
                y0[24] = initial_protected_passengers
            ...
            y0[25] = pintc * Nc  # Initial no. protected crew
    ...
    def the_ode_model(t, y, bbarcp, cpp, b1, tpp, tcc, thetaa, thetap, chi, effp, effc):
        ...
        ft = Ntests /(np.sum(y[[0, 1, 2, 3, 6, 7, 8, 9, 16, 17, 18, 19, 24, 25]]))
        ...
        dy[2] = y[0] * (bpp * thetaa * y[8] + bpp * thetap * y[6] + bpp * y[10] + bpp * y[14]) / 2666 + \
                y[0] * (bpc * thetaa * y[9] + bpc * thetap * y[7] + bpc * y[11] + bpc * y[15]) / 1045 + \
                y[24]*(1-effp)*(bpp*thetaa*y[8]+ bpp*thetap*y[6]+ bpp*y[10]+ bpp*y[14])/2666 + \
                y[24]*(1-effp)*(bpc*thetaa*y[9]+ bpc*thetap*y[7]+ bpc*y[11]+ bpc*y[15])/1045 - nu * y[2]
        dy[3] = y[1] * (bcp * thetaa * y[8] + bcp * thetap * y[6] + bcp * y[10] + bcp * y[14]) / 2666 + \
                y[1] * (bcc * thetaa * y[9] + bcc * thetap * y[7] + bcc * y[11] + bcc * y[15]) / 1045 + \
                y[25]*(1-effc)*(bcp*thetaa*y[8]+ bcp*thetap*y[6]+ bcp*y[10]+ bcp*y[14])/2666 + \
                y[25]*(1-effc)*(bcc*thetaa*y[9]+bcc*thetap*y[7]+bcc*y[11]+bcc*y[15])/1045 - nu * y[3]
        dy[4] = nu * y[2] - nu * y[4]
        dy[5] = nu * y[3] - nu * y[5]
        ...
        dy[22] = nu*y[4]
        dy[23] = nu*y[5]
        dy[24] = -y[24]*(1-effp)*(bpp*thetaa*y[8]+ bpp*thetap*y[6]+ bpp*y[10]+ bpp*y[14])/2666 - \
                y[24]*(1-effp)*(bpc*thetaa*y[9]+ bpc*thetap*y[7]+ bpc*y[11]+ bpc*y[15])/1045
        dy[25] = -y[25]*(1-effc)*(bcp*thetaa*y[8]+ bcp*thetap*y[6]+ bcp*y[10]+ bcp*y[14])/2666 - \
                y[25]*(1-effc)*(bcc*thetaa*y[9]+bcc*thetap*y[7]+bcc*y[11]+bcc*y[15])/1045
        return dy